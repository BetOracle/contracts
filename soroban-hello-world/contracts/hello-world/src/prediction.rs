...
impl BetOraclePrediction {
    ...
    pub fn set_agent_wallet(env: &Env, agent_wallet: Address) {
        ...
        let old_wallet: Address = if env.storage().instance().has(&DataKey::AgentWallet) {
            env.storage().instance().get::<DataKey, Address>(&DataKey::AgentWallet).unwrap()
        } else {
            Address::from_string(&String::from_str(&env, "GAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAWH2"))
        };
    ...
    fn _store_prediction(
        ...
        let mut prediction_ids: Vec<Bytes> = if env.storage().instance().has(&DataKey::PredictionIds) {
            env.storage().instance().get::<DataKey, Vec<Bytes>>(&DataKey::PredictionIds).unwrap()
        } else {
            Vec::new(&env)
        };
        ...
        let mut total_predictions: u64 = if env.storage().instance().has(&DataKey::TotalPredictions) {
            env.storage().instance().get::<DataKey, u64>(&DataKey::TotalPredictions).unwrap()
        } else {
            0
        };
        total_predictions += 1;
    ...
    pub fn submit_prediction_with_stake(
        ...
        // Update total staked
        let mut total_staked: u64 = if env.storage().instance().has(&DataKey::TotalStaked) {
            env.storage().instance().get::<DataKey, u64>(&DataKey::TotalStaked).unwrap()
        } else {
            0
        };
    ...
    pub fn resolve_prediction(env: &Env, prediction_id: Bytes, outcome: u32) {
        ...
        let mut pred: Prediction = env.storage().instance().get::<DataKey, Prediction>(&DataKey::Prediction(prediction_id.clone())).unwrap();
        if pred.timestamp == 0 {
        ...
        let mut resolved_predictions: u64 = if env.storage().instance().has(&DataKey::ResolvedPredictions) {
            env.storage().instance().get::<DataKey, u64>(&DataKey::ResolvedPredictions).unwrap()
        } else {
            0
        };
        ...
        if correct {
            let mut correct_predictions: u64 = if env.storage().instance().has(&DataKey::CorrectPredictions) {
                env.storage().instance().get::<DataKey, u64>(&DataKey::CorrectPredictions).unwrap()
            } else {
                0
            };
    ...
    pub fn get_predictions(env: &Env, offset: u64, limit: u64) -> Vec<Prediction> {
        let prediction_ids: Vec<Bytes> = if env.storage().instance().has(&DataKey::PredictionIds) {
            env.storage().instance().get::<DataKey, Vec<Bytes>>(&DataKey::PredictionIds).unwrap()
        } else {
            Vec::new(&env)
        };
    ...
    pub fn get_agent_accuracy(env: &Env) -> u64 {
        let resolved_predictions: u64 = if env.storage().instance().has(&DataKey::ResolvedPredictions) {
            env.storage().instance().get::<DataKey, u64>(&DataKey::ResolvedPredictions).unwrap()
        } else {
            0
        };
        ...
        let correct_predictions: u64 = if env.storage().instance().has(&DataKey::CorrectPredictions) {
            env.storage().instance().get::<DataKey, u64>(&DataKey::CorrectPredictions).unwrap()
        } else {
            0
        };
