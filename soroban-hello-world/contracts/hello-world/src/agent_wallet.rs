...
impl BetOracleAgentWallet {
    ...
    pub fn update_metadata(env: &Env, metadata_uri: String) {
        ...
        let mut profile: AgentProfile = env.storage().instance().get::<DataKey, AgentProfile>(&DataKey::Profile).unwrap();
    ...
    pub fn submit_prediction(
        ...
        let owner: Address = env.storage().instance().get::<DataKey, Address>(&DataKey::Owner).unwrap();
        let is_authorized = env.storage().instance().has(&DataKey::AuthorizedBackend(env.current_contract_address()))
            || env.current_contract_address() == owner;
        if !is_authorized {
            panic!("Not authorized");
        }

        let prediction_contract: Address = env.storage().instance().get::<DataKey, Address>(&DataKey::PredictionContract).unwrap();
        let zero_addr = Address::from_string(&String::from_str(&env, "GAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAWH2"));
        if prediction_contract == zero_addr {
            panic!("Prediction contract not set");
        }

        let profile: AgentProfile = env.storage().instance().get::<DataKey, AgentProfile>(&DataKey::Profile).unwrap();
        if !profile.active {
        ...
        let mut reputation: Reputation = if env.storage().instance().has(&DataKey::Reputation) {
            env.storage().instance().get::<DataKey, Reputation>(&DataKey::Reputation).unwrap()
        } else {
            Reputation {
                total_predictions: 0,
    ...
    pub fn process_payment(
        ...
        if env.storage().instance().has(&DataKey::ProcessedPayment(payment_id.clone())) {
    ...
    pub fn get_profile(env: &Env) -> AgentProfile {
        env.storage().instance().get::<DataKey, AgentProfile>(&DataKey::Profile).unwrap()
    }
