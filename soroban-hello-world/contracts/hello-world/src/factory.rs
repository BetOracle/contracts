...
impl BetOracleFactory {
    ...
    fn _deploy_agent_wallet_internal(
        ...
        let agent_wallet = env.invoker_contract();
        ...
        env.events()
            .publish(("agent_deployed", agent_id.clone(), agent_wallet.clone()), (name.clone(), env.invoker_contract(), env.ledger().timestamp()));
    ...
    fn _deploy_prediction_contract_internal(env: &Env, agent_wallet: Address) -> Address {
        ...
        let prediction_contract = env.invoker_contract();
    ...
    pub fn deploy_full(
        ...
        env.events()
            .publish(("full_deployment", agent_id.clone(), agent_wallet.clone(), prediction_contract.clone()), (env.invoker_contract(), env.ledger().timestamp()));
