...
contract BetOraclePrediction {
    ...
    event PredictionResolved(
        bytes32 indexed predictionId, bytes32 indexed matchId, uint8 outcome, bool correct, uint256 timestamp
    );
    ...
    function submitPrediction(
        ...
        _storePrediction(
            _predictionId, _matchId, _homeTeam, _awayTeam, _league, _prediction, _confidence, _matchDate, 0
        );
    ...
    function _storePrediction(
        ...
        emit PredictionSubmitted(
            _predictionId, _matchId, agentWallet, _homeTeam, _awayTeam, _prediction, _confidence, block.timestamp
        );
    }
    ...
    function submitPredictionWithStake(
        ...
        _storePrediction(
            _predictionId, _matchId, _homeTeam, _awayTeam, _league, _prediction, _confidence, _matchDate, msg.value
        );
     ...
     */
    function resolvePrediction(bytes32 _predictionId, uint8 _outcome) external onlyAgent {
        ...
        if (pred.matchDate > 0) {
            require(block.timestamp >= pred.matchDate - 1 hours, "Match has not started yet");
        }
        ...
        emit PredictionResolved(_predictionId, pred.matchId, _outcome, correct, block.timestamp);
    }
     ...
     */
    function getPredictions(uint256 _offset, uint256 _limit) external view returns (Prediction[] memory) {
     ...
     */
    function generatePredictionId(bytes32 _matchId, uint256 _timestamp) external pure returns (bytes32) {
