...
contract BetOracleAgentWallet is Ownable {
    ...
    event AgentRegistered(bytes32 indexed agentId, string name, address indexed owner, uint256 timestamp);

    event BackendAuthorized(address indexed backend, bool authorized);

    event PredictionSubmitted(
        bytes32 indexed predictionId, bytes32 indexed matchId, uint256 confidence, uint256 timestamp
    );

    event ReputationUpdated(uint256 newScore, uint256 totalPredictions, uint256 correctPredictions, uint256 timestamp);

    event PaymentProcessed(bytes32 indexed paymentId, address indexed token, uint256 amount, uint256 timestamp);

    // ============ Modifiers ============

    modifier onlyAuthorized() {
        require(msg.sender == owner() || authorizedBackends[msg.sender], "Not authorized");
        _;
    }

    // ============ Constructor ============

    constructor(bytes32 _agentId, string memory _name, string memory _metadataURI, address _owner) Ownable(_owner) {
        require(_agentId != bytes32(0), "Invalid agent ID");
        require(bytes(_name).length > 0, "Name required");

        profile = AgentProfile({
            agentId: _agentId, name: _name, metadataURI: _metadataURI, createdAt: block.timestamp, active: true
        });
     ...
     */
    function updateReputation(uint256 _totalPredictions, uint256 _correctPredictions) external onlyOwner {
        ...
        emit ReputationUpdated(reputation.reputationScore, _totalPredictions, _correctPredictions, block.timestamp);
    }
     ...
     */
    function processPayment(bytes32 _paymentId, address _token, uint256 _amount, address _recipient)
        external
        onlyAuthorized
        returns (bool)
    {
