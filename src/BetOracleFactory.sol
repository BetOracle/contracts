...
contract BetOracleFactory {
    ...
    event AgentDeployed(
        bytes32 indexed agentId, address indexed agentWallet, string name, address indexed owner, uint256 timestamp
    );

    event PredictionContractDeployed(
        address indexed predictionContract, address indexed agentWallet, uint256 timestamp
    );
     ...
     */
    function deployAgentWallet(bytes32 _agentId, string calldata _name, string calldata _metadataURI)
        external
        returns (address agentWallet)
    {
     ...
     */
    function _deployAgentWalletInternal(bytes32 _agentId, string memory _name, string memory _metadataURI)
        internal
        returns (address agentWallet)
    {
        require(agentById[_agentId] == address(0), "Agent ID already exists");

        agentWallet = address(new BetOracleAgentWallet(_agentId, _name, _metadataURI, msg.sender));
     ...
     */
    function deployPredictionContract(address _agentWallet) external returns (address predictionContract) {
     ...
     */
    function _deployPredictionContractInternal(address _agentWallet) internal returns (address predictionContract) {
        ...
        // NOTE: Must be called by the agent wallet owner (not the factory).
     ...
     */
    function deployFull(bytes32 _agentId, string calldata _name, string calldata _metadataURI)
        external
        returns (address agentWallet, address predictionContract)
    {
        // Deploy agent
        ...
        emit FullDeployment(_agentId, agentWallet, predictionContract, msg.sender, block.timestamp);
